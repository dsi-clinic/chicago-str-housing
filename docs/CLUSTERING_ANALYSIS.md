...
- **Pre-processing**: Step 3 filters the data to tracts used in clustering. Step 4 calculates Moran's I for spatial autocorrelation and optionally renders a pairwise scatterplot matrix (run the script with `--scatter-matrix` to include this diagnostic).
- **Clustering Methods**:  
...
- **Outputs**: Each clustering solution produces maps, cluster profiles, size plots, attribute distributions, and statistical summaries.
- **Evaluation Metrics**:  
...
| `kmeans_cluster_profiles.png`, `ward*_cluster_profiles.png` | Tables summarizing cluster counts and mean attribute values. | |
| `kmeans_cluster_sizes.png`, `ward*_cluster_sizes.png` | Bar charts displaying cluster membership counts. | Quickly assess cluster balance/imbalance. Bars are annotated with integer counts. |
| `kmeans_attribute_distributions.png`, `ward*_attribute_distributions.png` | Faceted KDE plots of each variable stratified by cluster. | Inspect distribution overlap (separation) across clusters for each attribute. |
...
| `kmeans_elbow_plot.png` + `kmeans_elbow_scores.csv` | K-means inertia across tested k values (k=2 to k=10). | Use the bend ("elbow") to justify the default k=6; the CSV lists exact inertia values. |
| `kmeans_ward_comparison_map.png` | Side-by-side K-means vs. Ward (unconstrained) choropleths. | Compare spatial footprints directly. |
