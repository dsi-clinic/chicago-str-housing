...
# Optional: treatment definition — threshold (default), binary (first prohibition month), or both
...
- `CENSUS_API_KEY` recommended for ACS covariates in `DIDCovariateProcessor`. Resolution order is: explicit loader argument (rare), then `CENSUS_API_KEY`, then a bundled demo key with a warning (see [`census_data.py`](../src/housing/components/loaders/census_data.py)).
...
## Treatment definition (`DID_TREATMENT_MODE`)

The tract-month panel uses either:

| Mode | Processor | Definition |
| --- | --- | --- |
| `threshold` (default) | `TreatmentThresholdProcessor` | Treated once unit-restricted share × ACS occupied units crosses a calendar-year percentile threshold (see code for fallback when only one group exists). |
| `binary` | `TreatmentIndicatorProcessor` | Treated from the tract’s first prohibition month (any adoption). |

Set `DID_TREATMENT_MODE=both` to run the **entire** pipeline twice and write **separate** trees: `{DID_CS_OUTPUT_DIR}-threshold` and `{DID_CS_OUTPUT_DIR}-binary` (suffix on the last path segment). The returned `(pipeline, context)` tuple refers to the **second** run (`binary`) for backward compatibility; inspect logs for the first run’s output path.

## Data path: rents and crosswalk

The main CS script builds tract-level rents with **`TimeSeriesRentalLoader`** (ZORI) → **`ZipTractCrosswalkProcessor`** (geometric weights) → **`TimeSeriesZipToTractProcessor`**, which exposes the panel as **`tract_panel_data`** in context (legacy alias `tract_rental_panel` is not required for this path).
...
- Sample / matching story (every CS run): `did_story_*.png` plus `did_story_*.csv` — tract funnel, pre-trend scatter, Love-style SMD plot, control reuse, cumulative adoption, pre-rent violins; see [`docs/methodology-audit/README.md`](methodology-audit/README.md).

Stable filenames are intended for Beamer under [`docs/white-paper/ppt/`](white-paper/ppt/).

## Methodology audit notes

Crosswalk and matching refactors are documented under
[`docs/methodology-audit/`](methodology-audit/README.md).

In particular:

- ZIP-to-tract interpolation now relies on projected, tract-oriented area shares.
- Trend matching now uses both pre-treatment slope and pre-treatment rent level.
- Supporting diagnostics are exported alongside the white-paper artefacts.
