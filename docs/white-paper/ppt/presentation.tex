...
\begin{frame}{Step 2 --- Why shrink controls? (\emph{pre-treatment matching})}
\begin{itemize}\setlength\itemsep{0.4em}
  \item Compared neighbourhoods differ in baseline prices and rent growth; we nonetheless need untreated tracts whose \emph{pre-prohibition rent path} resembles treated tracts' paths as closely as feasible.
  \item \textbf{Procedure:} for each treated tract, retain up to three never-treated tracts with closest standardized pre-period rent slope and average pre-period rent ($k{=}3$ nearest neighbours); require $\geq 6$ pre months.
  \item \textbf{Treated preservation:} \emph{all} prohibition tracts survive this filter; removals apply only to never-treated tracts that never serve as usable counterfactual matches.
\end{itemize}
\end{frame}
