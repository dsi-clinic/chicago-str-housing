"""STR buildings count distribution visualizer.

This module creates visualizations showing STR buildings count distributions
...
class STRBuildingsCountDistributionVisualizer(Visualizer):
    """Create visualizations for STR buildings distributions.

    Shows STR buildings list aggregated at both census tract and community area
    levels to demonstrate the hierarchical spatial aggregation.
    """

    def __init__(self, output_dir: str | None = None) -> None:
        """Initialize the STR buildings distribution visualizer.

        Args:
            output_dir: Optional output directory for visualizations
        """
        super().__init__(
            "str_buildings_count_distribution_visualization",
            "Create visualizations for STR buildings count distributions",
        )
        self.output_dir = output_dir or "/project/output"

    def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        """Create STR buildings count distribution visualizations."""
        logger.info("Creating STR buildings count distribution visualizations...")
        ...
        fig.suptitle(
            "Chicago STR Building Count Distribution: Tract vs Community Area",
            fontsize=18,
            fontweight="bold",
        )

        # 1. Tract-level STR buildings count distribution
        if tract_data is not None:
            tract_counts = tract_data["str_buildings_count"].dropna()
            axes[0, 0].hist(
                tract_counts,
                bins=30,
                alpha=0.7,
            ...
            axes[0, 0].set_xlabel("STR Buildings Point Count", fontsize=12)
        ...
        # 2. Community-level STR buildings count distribution
        if community_data is not None:
            community_counts = community_data["str_buildings_count"].dropna()
            axes[0, 1].hist(
                community_counts,
                bins=20,
                alpha=0.7,
            ...
            axes[0, 1].set_xlabel("STR Buildings Point Count", fontsize=12)
        ...
        # 3. Box plots for STR buildings densities
        if tract_data is not None and community_data is not None:
            ...
            axes[1, 0].set_ylabel("STR Buildings Point Count", fontsize=12)
            axes[1, 0].set_title("STR Buildings Count Comparison", fontsize=14)
            axes[1, 0].grid(True, alpha=0.3, axis="y")

        # 4. Statistical summary
        axes[1, 1].axis("off")
        stats_lines = ["STR Buildings Statistics\n" + "=" * 40 + "\n"]
        ...
        # Save the plot
        output_path = (
            Path(self.output_dir) / "str_buildings_count_distribution_analysis.png"
        )
