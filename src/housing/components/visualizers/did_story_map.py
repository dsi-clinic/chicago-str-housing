...
# Cohort year bucket boundaries (calendar years)
YEAR_EARLY_END = 2016
YEAR_MID_END = 2018
YEAR_LATE_END = 2020
...
def _add_north_arrow(ax: Axes) -> None:
...
class DiDStoryMapVisualizer(Visualizer):
    ...
    def __init__(self, output_dir: str | Path | None = None) -> None:
        """Configure output directory for editorial DiD sample maps."""
        super().__init__(
    ...
    def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        """Render matched-sample status and cohort choropleth maps."""
        ...
        merged["status"] = merged["ever_treated"].apply(
            lambda v: "Treated"
            if v == 1
            else ("Matched control" if v == 0 else "Out of sample")
        )
        ...
        if city_boundaries is not None:
            ...
            try:
                ...
                logger.warning(
                    "did_story_map: city intersect failed (%s); using all tracts", exc
                )
    ...
    def _panel_status(
        ...
        out_of_sample.plot(
            ax=ax, color=OUT_OF_SAMPLE, edgecolor="white", linewidth=0.1, zorder=1
        )
        controls.plot(
            ax=ax,
            color=CONTROL,
        ...
        legend_handles = [
            mpatches.Patch(
                facecolor=TREATED,
                edgecolor="white",
                label="Treated tract (ever STR prohibition)",
            ),
            mpatches.Patch(
                facecolor=CONTROL,
                edgecolor="white",
                label="Matched never-treated control",
            ),
            mpatches.Patch(
                facecolor=OUT_OF_SAMPLE,
                edgecolor="#bbbbbb",
                label="Tract outside matched panel",
            ),
        ]
        ax.legend(
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _frame_axes(ax: Axes, bounds: MapBounds, title: str) -> None:
