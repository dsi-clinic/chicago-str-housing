...
class ForeclosedMapVisualizer(Visualizer):
    ...
    def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        community_data = context.get("community_foreclosed_data")
        ...
        if tract_data is not None and tract_boundaries is not None:
            ...
            axes[0].set_title(
                f"Census Tract Level (n={tract_data['point_count'].notna().sum()})",
                fontsize=16,
            )
            axes[0].axis("off")
        ...
        if community_data is not None and community_boundaries is not None:
            community_map_data = community_boundaries.merge(
                community_data[["community_name", "point_count"]],
                on="community_name",
                how="left",
            )
            ...
            axes[1].set_title(
                f"Community Area Level (n={community_data['point_count'].notna().sum()})",
                fontsize=16,
            )
            axes[1].axis("off")
