...
class TimeSeriesZipToTractProcessor(DataProcessor):
    ...
    def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info("Matched %d zip codes with tracts", len(zip_tract_merge["zip_code"].unique()))

        #Calculate the spatial-weighted average rents for each tract
        zip_tract_merge["weighted_rental_price"] = zip_tract_merge["rental_price"] * zip_tract_merge["intersection_area"]
        tract_panel = (
        ...
        tract_panel["rental_price"] = tract_panel["weighted_rental_price"] / tract_panel["intersection_area"]
