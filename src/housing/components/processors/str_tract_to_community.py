...
class STRTractToCommunityProcessor(DataProcessor):
    """Aggregate STR tract-level data up to community areas."""

    def __init__(self) -> None:
        super().__init__(
            "str_tract_to_community", "Aggregate STR tract data to community areas"
        )

    def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        logger.info("Aggregating STR tract data to community areas...")

        # Grab STR tract data and community boundaries
        tract_data = context["str_tract_data"]
        ...
        final_result["point_count"] = final_result["point_count"].fillna(0)

        # Compute density (listings per km²)
        final_result["point_density"] = (
            final_result["point_count"] / final_result["area_km2"]
        )
        ...
        logger.info(
            ...
            final_result["point_density"].min(),
            final_result["point_density"].max(),
        )

        logger.info("Final result: %d community areas", len(final_result))

        return {"community_str_data": final_result}
