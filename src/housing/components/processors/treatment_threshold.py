...
class TreatmentThresholdProcessor(DataProcessor):
    ...
    def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        tract_panel_data["month"] = pd.to_datetime(tract_panel_data["month"]).dt.to_period("M").dt.to_timestamp()
        str_tract_data["prohibition_month"] = pd.to_datetime(str_tract_data["prohibition_date"]).dt.to_period("M").dt.to_timestamp()

        prohibited_units_by_month = (
            str_tract_data[["tract_geoid", "prohibition_month", "number_of_units"]]
            .rename(
                columns={
        ...
        merged["treated_units_in_month"] = (
            merged["treated_units_in_month"].fillna(0)
        )

        # Merge census total occupied housing units (need to find percentage of units restricted)
        if census_data is not None and "total_occupied_units" in census_data.columns:
            ...
            census_tract = census_tract.rename(columns={"total_occupied_units": "total_occupied_units"})
            merged = merged.merge(
                census_tract,
                left_on="tract_geoid",
                right_on="tract_id",
            ...
            logger.warning("No census_data or total_occupied_units; threshold uses units only.")

        merged = merged.sort_values(["tract_geoid", "month"])
        merged["cumulative_treated_units"] = merged.groupby("tract_geoid")["treated_units_in_month"].cumsum()
        ...
        merged["pct_units_restricted"] = (prohibited_units / total_occupied_units).fillna(0).astype(float)
        
        # Calculating distribution of pct_units_restricted
        ...
        merged["threshold"] = merged["threshold"].fillna(0)

        # Finding first month (per tract) where pct_units_restricted >= threshold for that year
        ...
        # treated = 1 if month >= treatment_start_month
        merged["treated"] = (
