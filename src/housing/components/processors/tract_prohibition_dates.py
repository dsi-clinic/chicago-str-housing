...
class TractProhibitionDatesProcessor(DataProcessor):
    ...
    def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        tract_prohibition_dates["first_prohibition_date"] = pd.to_datetime(
            tract_prohibition_dates["first_prohibition_date"]
        )

        logger.info(
            "Created STR tract summary with %d tracts", len(tract_prohibition_dates)
        )
        logger.info(
            "Tracts with prohibitions: %d",
            (tract_prohibition_dates["building_count"] > 0).sum(),
        )
