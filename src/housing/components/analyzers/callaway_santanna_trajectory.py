...
class CallawaySantAnnaTrajectoryAnalyzer(Analyzer):
    ...
    def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        if "rel_time" not in es.columns or "att" not in es.columns or "se" not in es.columns:
            raise ValueError("cs_event_study must have rel_time, att, se")

        post = es[es["rel_time"] >= 0].sort_values("rel_time").copy()
        post = post[np.isfinite(post["att"]) & np.isfinite(post["se"]) & (post["se"] > 0)]

        boot_df = context.get("cs_event_study_tract_bootstrap")
        phase_se_column = "se"
        boot_meta: dict[str, Any] = {}
        if isinstance(boot_df, pd.DataFrame) and not boot_df.empty and "se_boot" in boot_df.columns:
        ...
        if (
            ...
            late_minus_early_se = math.sqrt(early_se.iloc[0] ** 2 + late_se.iloc[0] ** 2)
            if late_minus_early_se > 0:
                late_minus_early_z = late_minus_early / late_minus_early_se
                late_minus_early_p = float(2 * (1 - stats.norm.cdf(abs(late_minus_early_z))))
        ...
        fits = {
            "linear": {
                ...
                "coefficient_se": {"intercept": float(math.sqrt(cov1[0, 0])), "slope_e": se_b1},
    ...
    def _classify(
        ...
        *,
        ...
        plateau_aic_log = (
            ...
            and (aic_log <= aic_quad + _TRAJECTORY_LOG_VS_QUAD_BAND or not math.isfinite(aic_quad))
        )
