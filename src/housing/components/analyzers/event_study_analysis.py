    ...
    def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        for k in event_window:
            if k == -1: # skip reference period
                continue
            did_panel[rel_time_col(k)] = (did_panel["months_since_treatment"] == k).astype(int)
        ...
        with Path(report_path).open("w") as f:
            ...
            f.write(f"Pre-treatment coefficients significant at 5%: {n_significant} / {len(pre_coefs)}\n\n")
