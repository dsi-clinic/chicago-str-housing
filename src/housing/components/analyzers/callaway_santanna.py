...
class CallawaySantAnnaAnalyzer(Analyzer):
    ...
    def _estimate_single_att(
        ...
        where g-1 is the pre-treatment period for cohort g.
        """
        # Define pre-treatment period (one period before cohort treatment)
        pre_period = cohort_date - pd.DateOffset(months=1)

        # Get data for treated cohort
        treated_current = df[
