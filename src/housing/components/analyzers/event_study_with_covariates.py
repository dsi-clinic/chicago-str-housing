    ...
    def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        for cov in potential_covariates:
            if cov in df.columns:
                # Check if covariate has variation and is not all missing
                if df[cov].notna().sum() > 0 and df[cov].nunique() > 1:
                    # Standardize covariate (mean=0, sd=1) for interpretability
                    covariate_data[cov] = (df[cov] - df[cov].mean()) / df[cov].std()
                    covariates_used.append(cov)
                    logger.info("Added covariate: %s (mean=%.2f, sd=%.2f)",
                               cov, df[cov].mean(), df[cov].std())
