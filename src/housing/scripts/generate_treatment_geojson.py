"""Join clustering_data.geojson with treatment profile CSVs.

Produces tracts_treatment.geojson for the interactive dashboard maps.
...
def generate(mode: str) -> None:
    """Build ``tracts_treatment.geojson`` for the given treatment-definition mode."""
    profile_path = (
        REPO_ROOT
        / f"output/did-cs-whitepaper-{mode}/did_story_treatment_tract_profile.csv"
    )
        ...
        features.append(
            {
                ...
                "properties": {
                    "tract_id": tid,
                    "ever_treated": int(info["ever_treated"])
                    if "ever_treated" in info
                    else -1,
                    "role": info.get("role", "not_in_sample"),
                    # Prohibition intensity for the overlay layer
                    "prohibition_units": int(
                        props.get("str_prohibition_units_total") or 0
                    ),
                    "prohibition_density": float(
                        props.get("str_prohibition_building_density") or 0.0
                    ),
